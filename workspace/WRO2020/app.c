...
void runBlueStreet(int backToBase) {
    ...
    ev3_motor_reset_counts(left_motor);
    ev3_motor_reset_counts(right_motor);
    ev3_motor_steer(left_motor, right_motor, -20, 2);
    ...
    ev3_motor_rotate(right_motor, 370, 30, true);
    tslp_tsk(100);
    ev3_motor_reset_counts(left_motor);
    ev3_motor_reset_counts(right_motor);
    ...
    if (instructions.doSnow == 1) {
        ...
        ev3_motor_steer(left_motor, right_motor, -10, 0);
        tslp_tsk(200);
        ev3_motor_rotate(a_motor, 200, 80, false);
        ev3_motor_rotate(right_motor, 235, 20, true);
        tslp_tsk(100);
        d_motor_index++;
    ...
    tslp_tsk(100);
    if (backToBase){
        ev3_motor_rotate(left_motor, 210, 20, true);
        tslp_tsk(100);
...
void runGreenStreet(int backToBase) {
    ...
    ev3_motor_reset_counts(left_motor);
    ev3_motor_reset_counts(right_motor);
    wall_follow_with_tasks(54, 3, 0, 1, 0, 30);
    tslp_tsk(100);
    ev3_motor_reset_counts(left_motor);
    ev3_motor_reset_counts(right_motor);
    ev3_motor_steer(left_motor, right_motor, -20, 2);
...
void runYellowStreet(int backToBase) {
    ...
    if(instructions.doCar == 0){
        ...
        if(instructions.doSnow){
            ...
            wall_follow_with_tasks(138,2,car,2,abrasive,45);
        }
    }
    else if(instructions.doCar == 1){
        ev3_motor_reset_counts(left_motor);
    ...
    if(backToBase){
        ...
        tslp_tsk(1000);
        ev3_motor_steer(left_motor,right_motor,0,0);
        ev3_motor_rotate(right_motor,10,10,false);
        ev3_motor_rotate(left_motor,10,-10,true);
    }
    else{
        //move forward
        ev3_motor_steer(left_motor,right_motor,30,0);
...
void runRedStreet(int backToBase) {
    ...
    if(instructions.doCar == 0){
        ...
        if(instructions.doSnow){
            wall_follow_with_tasks(130,3,car,3,abrasive,35);
        }
        else{
            a_motor_index = 1;
            wall_follow_with_tasks(130,3,car,1,abrasive,55);
        }
    }
    else if(instructions.doCar == 1){
        ev3_motor_reset_counts(left_motor);
    ...
    if(backToBase){
        ...
        ev3_motor_steer(left_motor,right_motor,0,0);
        if(instructions.doSnow && instructions.collectAbrasive){
            ev3_motor_steer(left_motor,right_motor,10,0);
            tslp_tsk(1000);
            ev3_motor_steer(left_motor,right_motor,0,0);
            ev3_motor_steer(left_motor,right_motor,-10,0);
...
/**
 * \brief follows a line using Color_3 and does tasks
 * \param distance Distance in cm
...
void color3PID(int distance,int tasksNumA,int tasksNumD){
    ...
    while (wheelDistance < distance) {
        ...
        wheelDistance = (ev3_motor_get_counts(left_motor) / 2 + ev3_motor_get_counts(right_motor) / 2) * ((3.1415926535 * 8.1) / 360);
        float error = ev3_color_sensor_get_reflect(color_3) - 47;
        integral = error + integral * 0.5;
        float steer = 0.6 * error + 0 * integral + 5 * (error - lasterror);
...
void wall_follow_with_tasks(int distance,int steer,int detectCar,int tasksNumA,int tasksNumD, int speed){
    ...
    while (wheelDistance < distance) {
        ...
        if(wheelDistance > next_d_motor_task[0] && tasksLeftD > 0){
            ev3_speaker_play_tone(NOTE_A4,60);
            ev3_motor_rotate(d_motor,360 * next_d_motor_task[1],40,false);
            d_motor_index += 1;
            for(int i = 0;i < 3;i++){
                next_d_motor_task[i] = allTasks[pos.street][2][d_motor_index][i];
            }
